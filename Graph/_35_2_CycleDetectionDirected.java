...
public class _35_2_CycleDetectionDirected {
    private int V; // Number of vertices
    private List<List<Integer>> adj; // Adjacency list

    // Constructor
    _35_2_CycleDetectionDirected(int v) {
        V = v;
        adj = new ArrayList<>();
        for (int i = 0; i < v; i++)
    ...
    boolean isCyclicUtil(int v, boolean visited[], boolean recStack[]) {
        if (recStack[v]) // Node is already in recursion stack â†’ Cycle detected
            return true;
    ...
    public static void main(String args[]) {
        _35_2_CycleDetectionDirected g = new _35_2_CycleDetectionDirected(4);
        g.addEdge(0, 1);
        g.addEdge(1, 2);
        g.addEdge(2, 3);
